...
def game() -> GameFactory:
    def _game(pgn, game_num: int = 0) -> CreateGifFromPGN:
...
def test_init(game: GameFactory):
...
def test_board_size(game: GameFactory):
...
@pytest.mark.parametrize(
    "method, var, val",
    [("add_analysis_bar", "_bar_size", 30), ("add_analysis_graph", "_graph_size", 81), ("enable_nags", "_nag", True)],
)
...
def test_square_colors(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    ...
    g.square_colors = {chess.WHITE: "#FF0000", chess.BLACK: "#00FF00"}  # ty: ignore[invalid-assignment]
    assert g.square_colors.square_color(chess.WHITE) == "#FF0000"
    assert g.square_colors.square_color(chess.BLACK) == "#00FF00"
    with pytest.raises(ValueError):
        g.square_colors = {chess.WHITE: "#FF0000", "BLACK": "#00FF00"}  # ty: ignore[invalid-assignment]
    with pytest.raises(ValueError):
        g.square_colors = []  # ty: ignore[invalid-assignment]


def test_piece_theme(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    g.piece_theme = PieceTheme.CASES
    assert g.piece_theme.value == "cases"
    with pytest.raises(ValueError):
        g.piece_theme = "cases"  # ty: ignore[invalid-assignment]


def test_headers(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
...
def test_reverse(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
...
def test_arrows(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
...
def test_generate_board_only(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    g.board_size = 240
    gif = g.generate()
...
def test_generate_board_headers(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    g.board_size = 240
    g.add_headers(25)
    gif = g.generate()
...
def test_generate_board_headers_reversed(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    g.board_size = 240
    g.add_headers(25)
    g.reverse_board()
...
def test_generate_en_passant(game: GameFactory):
...
def test_generate_capture(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS, 1)
    g.piece_theme = PieceTheme.CASES
    g.board_size = 240
    g.add_headers(25)
...
def test_generate_check_arrow(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS, 1)
...
def test_generate_eval_graph(game: GameFactory):
...
def test_generate_nags(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_EVAL_ANNOTATIONS)
    g.square_colors = BoardTheme("#000000", "#000000")
    g.board_size = 400
    g.enable_nags()
...
def test_generate_eval_bar(game: GameFactory):
    g: CreateGifFromPGN = game(PGN_EVAL_ANNOTATIONS)
    g.board_size = 240
...
def test_generate_save_image(game: GameFactory, tmpdir):
    g: CreateGifFromPGN = game(PGN_NO_ANNOTATIONS)
    g.board_size = 240
    temp_path = str(tmpdir.mkdir("gifpgn").join("gifpgn_test.gif"))
    gif = g.generate(temp_path)
    assert gif is None
